"""Idempotent injection of the sanity test individual into the registry database.
...
the sample individuals are only present when `dbSeed.loadSampleData=true`, which a
...
def ensure_seeded(cfg) -> str:
    """Insert (or reset) the sanity individual. Returns "seeded"."""
    from . import db
