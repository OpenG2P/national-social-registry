# docker/scripts/ — Local build scripts for the NSR Docker images
...
3. Copies any **local-path deps** (e.g. `./nsr-extension`) into
...
```bash
docker inspect --format '{{.Architecture}}' openg2p/openg2p-nsr-staff-portal-api:develop
# → amd64
```
...
./nsr-extension                                               ← local-path dep (relative to project root)
...
For **local-path** entries (e.g. `./nsr-extension`), `parse_service.py`
...
3. Update any git-branch references if pinning to a specific NSR extension tag
