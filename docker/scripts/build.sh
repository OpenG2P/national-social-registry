...
# workflows for the National Social Registry (NSR) repo.
#
...
if [[ "${BUILD_PLATFORM}" == *","* ]]; then
  ...
  if ! docker buildx inspect openg2p-nsr-builder &>/dev/null; then
    docker buildx create --name openg2p-nsr-builder --use
  else
    docker buildx use openg2p-nsr-builder
  fi
...
for SERVICE_FILE in "${SERVICE_FILES[@]}"; do
  ...
  BUILD_ARGS=(
    ...
    --label "org.opencontainers.image.description=OpenG2P National Social Registry service image"
  )
