-- Aggregates for the Maps surface at the third level below country (woreda in ETH, ward in XKM).
--
-- Named by DEPTH, not by level name. The reporting views unpack geography
-- positionally into geo_4/geo_4_id, so this query is identical for every
...
select
    h.geo_4_id       as pcode,
    h.geo_4    as area_name,
    h.geo_3_id   as parent_pcode,
    count(*)                                                              as households,
